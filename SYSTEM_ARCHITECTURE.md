...
All entrypoint scripts source `.env` before execution so OAuth credentials and overrides are available in child processes.
...
- `sjzl_daily_email.py` drives both lesson discovery and message composition.
  ...
  - Shared helpers handle UTF-8 decoding, optional OpenCC conversion, CSS aggregation, and `send_email`, which sends via Gmail API using `EMAIL_*` variables.
...
- `sjzl_daily_email.send_email()` is the single Gmail API integration point. It sends multipart/alternative messages using OAuth authentication and is reused by weekly summaries and reply confirmation messages.
...
- `scripts/process_schedule_replies.py` (invoked by `scripts/run_schedule_reply_processor.sh`) exposes CLI flags for batch size and dry runs when testing reply processing.
...
- `state/last_reply_results.json` — serialized results from the latest reply processing run.
...
- **Email delivery**: `SMTP_USER`, `EMAIL_FROM`, `EMAIL_TO`, `CONTENT_LANGUAGE`.
...
- **Gmail API**: OAuth-authenticated access to Gmail for sending multipart emails to subscribers and admin recipients.
